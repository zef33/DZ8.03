...
4. `Проверяем работу перенаправления запросов скриншот 4. В данном примере использует раздел конфигурации listen web_tcp для балансировки  на 4 уровне`
```
global
	log /dev/log	local0
...
3. `Прописываем домен example.local в конфигурационном файле` [haproxy.cfg](https://github.com/zef33/DZ8.03/blob/Balans/задание1/haproxy.cfg) `При запросе с другого адреса видим ошибку скриншот 4`
5. `На скриншоте 3 видим перенаправление запросов при обращении к HAProxy работающего на 7 уровне`
```
global
	log /dev/log	local0
	log /dev/log	local1 notice
